...
const WallpaperWindow = ({ isOpen, onClose, currentWallpaper, onSelect }: WallpaperWindowProps) => {
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl bg-[#1e1e1e]/90 backdrop-blur-xl border-white/10 text-white p-0 overflow-hidden outline-none">
        <div className="flex h-[450px]">
          {/* Sidebar */}
          <div className="w-1/3 bg-black/20 p-4 border-r border-white/5">
            <h3 className="text-xs font-bold text-white/40 uppercase tracking-wider mb-4">Appearance</h3>
            <div className="bg-blue-600/20 text-blue-400 px-3 py-1.5 rounded-md text-sm font-medium">
              Wallpaper
            </div>
          </div>
